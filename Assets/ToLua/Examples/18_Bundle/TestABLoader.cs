...
public class TestABLoader : MonoBehaviour
{
    ...
    IEnumerator CoLoadBundle(string name, string path)
    {
        using (UnityWebRequest www = new UnityWebRequest(path))
        {
            ...
            --bundleCount;
            LuaFileUtils.Instance.AddSearchBundle(name, (www.downloadHandler as DownloadHandlerAssetBundle).assetBundle);
            www.Dispose();
        }
    }
    ...
    public IEnumerator LoadBundles()
    {
        ...
        AssetBundleManifest manifest = (AssetBundleManifest)(www.downloadHandler as DownloadHandlerAssetBundle).assetBundle.LoadAsset("AssetBundleManifest");
